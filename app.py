...
if st.session_state.page == 'DASHBOARD':
    ...
    # EQUITY CURVE
    st.markdown("<div class='ticker-label'>EQUITY_CURVE // PERFORMANCE</div>", unsafe_allow_html=True)
    ...
    with c2:
        ...
        if not bal.empty:
            for p in bal['portfolio'].unique():
                st.markdown(f"<div style='font-size:10px; color:#0070FF; margin-top:5px;'>{p}</div>", unsafe_allow_html=True)
...
# --- 7. PAGINA: TRADE EXECUTION (LEDGER MODIFICABILE) ---
elif st.session_state.page == 'TRADE':
    st.markdown("### / EXECUTION_LOG")
    
    # 1. Form di inserimento rapido
    with st.expander("NEW_TRADE_ENTRY", expanded=False):
        with st.form("t_form", clear_on_submit=True):
            ...
            if st.form_submit_button("COMMIT_NEW_TRADE"):
                ...
                supabase.table("trades").insert({"asset": asset, "instrument": instr, "shares": shares, "leverage": lev, "currency": curr, "portfolio": acc, "side": side, "entry_price": entry, "fees": fees, "cost": cost, "notional": entry * shares, "status": "OPEN", "date": str(datetime.date.today())}).execute()
                st.rerun()

    # 2. Ledger Modificabile
    if not trades.empty:
        # Check colonna close_date
        if 'close_date' not in trades.columns: trades['close_date'] = None
        
        st.markdown("<div class='ticker-label'>INTERACTIVE_LEDGER (Edit & Save)</div>", unsafe_allow_html=True)
        edited_trades = st.data_editor(trades, use_container_width=True, hide_index=True, disabled=["id", "cost", "notional", "profit", "pnl_perc", "date"], 
                                      column_config={"status": st.column_config.SelectboxColumn("STATUS", options=["OPEN", "CLOSED"]), "close_date": st.column_config.DateColumn("CLOSE_DATE")})
        ...
        if st.button("SAVE_LEDGER_CHANGES"):
            try:
                for _, row in edited_trades.iterrows():
                    ...
                    # Update data
                    update_payload = {"status": row['status'], "exit_price": row['exit_price'], "profit": pnl, "pnl_perc": p_perc}
                    if 'close_date' in trades.columns: update_payload["close_date"] = str(row['close_date']) if row['close_date'] else None
                    
                    supabase.table("trades").update(update_payload).eq("id", row['id']).execute()
                st.success("SYNCED"); st.rerun()
            except Exception as e: st.error(f"UPDATE_ERROR: {e}")
